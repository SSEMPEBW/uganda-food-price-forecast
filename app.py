...
try:
    ...
    if rain_df is not None and rain_df['Rainfall_mm'].sum() < 50:
        st.error(f"🌵 **Drought Risk**: Only {rain_df['Rainfall_mm'].sum():.0f}mm rain expected")
    elif rain_df is not None and rain_df['Rainfall_mm'].sum() > 300:
        st.info(f"🌊 **Heavy Rain**: {rain_df['Rainfall_mm'].sum():.0f}mm expected")
    ...
    if rain_df is not None:
        ax2.bar(rain_df['Date'], rain_df['Rainfall_mm'], color='#4682B4', alpha=0.7)
